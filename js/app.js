...
// ---------- assessor calendar (which days had revision, per subject) ----------

// A subject's average — whether one day's worth (the calendar's own red
// marker, per session-day) or its all-time figure (the overall-averages
// dials, below) — is flagged whenever it sits below this.
const CALENDAR_LOW_THRESHOLD = 40;

// Groups a profile's full result history by calendar day, then by subject
// within that day, into { count, totalPct } — everything the calendar and
// its per-day detail modal need. Built once per renderAssessorProfile() call
// from the results already fetched for that screen (no extra storage call),
// then reused across every month the parent navigates to without recomputing.
function computeAssessorCalendarByDay(results) {
  const byDay = {};
  results.forEach((r) => {
    const day = new Date(r.timestamp).toDateString();
    if (!byDay[day]) byDay[day] = {};
    if (!byDay[day][r.subject]) byDay[day][r.subject] = { count: 0, totalPct: 0 };
    byDay[day][r.subject].count += 1;
    byDay[day][r.subject].totalPct += r.percentage;
  });
  return byDay;
}

// Each subject's ALL-TIME average and session count, independent of any
// single day — feeds the "Overall averages" dials, which are the app's
// answer to "is this subject trending badly overall" (as opposed to the
// calendar's per-day marker, which answers "was this specific day bad").
// Kept as a genuinely separate figure/widget rather than folded into the
// calendar, after the calendar's per-day marker and this all-time trend
// figure turned out to be two different things a parent wants to see, not
// one thing rendered two ways.
function computeAssessorSubjectOverallAverages(results) {
  const bySubject = {};
  results.forEach((r) => {
    if (!bySubject[r.subject]) bySubject[r.subject] = { count: 0, totalPct: 0 };
    bySubject[r.subject].count += 1;
    bySubject[r.subject].totalPct += r.percentage;
  });
  const averages = {};
  Object.entries(bySubject).forEach(([subjectKey, s]) => {
    averages[subjectKey] = { avg: Math.round(s.totalPct / s.count), count: s.count };
  });
  return averages;
}

// A small semicircular "dial" per subject showing its all-time average —
// the arc fills left-to-right in proportion to the percentage, using the
...
function buildAssessorSubjectDialsHtml(subjectOverallAvg) {
  ...
  const dials = Object.values(SUBJECTS)
    ...
    .map((subject) => {
      ...
      return `
        <div class="assessor-dial">
          <svg viewBox="0 0 120 65" class="assessor-dial-svg">
            <path d="M 10 60 A 50 50 0 0 1 110 60" class="assessor-dial-track" />
            <path d="M 10 60 A 50 50 0 0 1 110 60" class="assessor-dial-fill" style="stroke:${color}; stroke-dasharray:${circumference.toFixed(2)}; stroke-dashoffset:${offset.toFixed(2)};" />
          </svg>
          <div class="dial-pct" style="color:${color};">${avg}%</div>
          <div class="dial-subject" style="color:${SUBJECT_COLOR[subject.key]};">${escapeHtml(subject.name)}</div>
          <div class="dial-meta">${count} session${count === 1 ? "" : "s"} overall</div>
        </div>
      `;
    })
    .join("");
  return dials || `<div class="empty-state">No sessions yet.</div>`;
}

// Renders one month's grid for the assessor calendar (nav header + weekday
// row + day cells). A day with any sessions gets one small dot per subject
// practised that day, coloured to match that subject everywhere else in the
// app (SUBJECT_COLOR); a dot gets a red ring when THAT DAY's own average for
// that subject is below CALENDAR_LOW_THRESHOLD — a per-day fact, deliberately
// separate from the all-time trend the dials above show. Days with no
// sessions are blank, not a "missed" marker — unlike the student's own
// home-screen calendar, this view has no "should have practised" expectation
// baked in.
function buildAssessorCalendarInnerHtml(byDay, year, month) {
  const daysInMonth = new Date(year, month + 1, 0).getDate();
  const startWeekday = (new Date(year, month, 1).getDay() + 6) % 7; // Monday = 0
  const now = new Date();
  ...
  let cells = "";
  for (let i = 0; i < startWeekday; i++) cells += `<div class="assessor-cal-cell empty"></div>`;
  for (let d = 1; d <= daysInMonth; d++) {
    const date = new Date(year, month, d);
    const dateStr = date.toDateString();
    const isToday = dateStr === todayStr;
    const isFuture = date.getTime() > new Date(todayStr).getTime();
    const daySubjects = !isFuture ? byDay[dateStr] : null;
    const hasData = !!daySubjects;

    const dotsHtml = hasData
      ? Object.values(SUBJECTS)
          .filter((subject) => daySubjects[subject.key])
          .map((subject) => {
            const s = daySubjects[subject.key];
            const dayAvg = Math.round(s.totalPct / s.count);
            const low = dayAvg < CALENDAR_LOW_THRESHOLD;
            return `<span class="assessor-cal-dot ${low ? "low" : ""}" style="background:${SUBJECT_COLOR[subject.key]};"></span>`;
          })
          .join("")
      : "";

    const cls = ["assessor-cal-cell"];
    if (isToday) cls.push("today");
    if (isFuture) cls.push("future");
    if (hasData) cls.push("has-data");

    cells += `
      <button type="button" class="${cls.join(" ")}" ${hasData ? `data-cal-day="${dateStr}"` : "disabled"}>
        <span class="cal-day">${d}</span>
        <span class="assessor-cal-dots">${dotsHtml}</span>
      </button>
    `;
  }

  return `
    <div class="assessor-cal-nav">
      <button type="button" class="btn secondary small" id="assessor-cal-prev" aria-label="Previous month">&larr;</button>
      <strong>${escapeHtml(monthLabel)}</strong>
      <button type="button" class="btn secondary small" id="assessor-cal-next" aria-label="Next month" ${isCurrentMonth ? "disabled" : ""}>&rarr;</button>
    </div>
    <div class="cal-weekdays">${["M", "T", "W", "T", "F", "S", "S"].map((d) => `<div>${d}</div>`).join("")}</div>
    <div class="cal-grid">${cells}</div>
    <div class="assessor-cal-legend">
      ${Object.values(SUBJECTS)
        .map((subject) => `<span><span class="assessor-cal-dot" style="background:${SUBJECT_COLOR[subject.key]};"></span>${escapeHtml(subject.name)}</span>`)
        .join("")}
      <span><span class="assessor-cal-dot low" style="background:var(--muted);"></span>Below ${CALENDAR_LOW_THRESHOLD}% that day</span>
    </div>
  `;
}

// In-page modal (same pattern as every other modal in this file — see the
// comment above showProfileNameModal() for why native dialogs are avoided)
// showing exactly what a tapped calendar day covers: every subject practised
// that day, how many times, and the average score, with the
// below-CALENDAR_LOW_THRESHOLD ones called out in red — this is THAT DAY's
// own average, matching what the dot's ring means. For a subject's all-time
// trend instead, see the "Overall averages" dials above the calendar.
function showAssessorCalendarDayModal(dateStr, daySubjects) {
  const existing = document.getElementById("assessor-cal-day-overlay");
  if (existing) existing.remove();
  ...
  const rows = Object.values(SUBJECTS)
    ...
    .map((subject) => {
      ...
      const low = avg < CALENDAR_LOW_THRESHOLD;
      return `
        <div class="assessor-cal-day-row">
          <div style="font-weight:700; color:${SUBJECT_COLOR[subject.key]};">${escapeHtml(subject.name)}</div>
          <div style="font-size:0.9rem;">Completed ${s.count} time${s.count === 1 ? "" : "s"} — average <strong class="${low ? "assessor-cal-low-text" : ""}">${avg}%</strong>${low ? ` ⚠️ below ${CALENDAR_LOW_THRESHOLD}%` : ""}</div>
        </div>
      `;
    })
    .join("");
  ...
  const overlay = document.createElement("div");
  overlay.className = "modal-overlay";
  overlay.id = "assessor-cal-day-overlay";
  overlay.innerHTML = `
    <div class="modal-card">
      <div class="modal-header">
        <strong>${escapeHtml(dateLabel)}</strong>
        <button class="modal-close" id="assessor-cal-day-close" aria-label="Close">✕</button>
      </div>
      ${rows}
    </div>
  `;
  ...
  document.getElementById("assessor-cal-day-close").addEventListener("click", close);
  overlay.addEventListener("click", (e) => {
...
async function renderAssessorProfile(profileName) {
  ...
  // no repeat storage calls.
  const calendarByDay = computeAssessorCalendarByDay(results);
  const calendarSubjectOverallAvg = computeAssessorSubjectOverallAverages(results);
  const dialsSectionHtml = `
    <div class="card">
      <h3 style="margin-top:0;">Overall averages</h3>
      <div style="color:var(--muted); font-size:0.85rem; margin-top:-6px; margin-bottom:14px;">Each subject's all-time average, across every session ever recorded.</div>
      <div class="assessor-dials">${buildAssessorSubjectDialsHtml(calendarSubjectOverallAvg)}</div>
    </div>
  `;
  const calendarSectionHtml = `
    <div class="card">
      <h3 style="margin-top:0;">Revision calendar</h3>
      <div style="color:var(--muted); font-size:0.85rem; margin-top:-6px; margin-bottom:10px;">Tap a day to see which subjects were practised, how many times, and that day's average score. A red-ringed dot means that subject averaged below ${CALENDAR_LOW_THRESHOLD}% on that specific day — for the all-time trend instead, see the dials above.</div>
      <div id="assessor-cal-wrap"></div>
    </div>
  `;
  ...
  // The calendar keeps its own little bit of view state (which month is
  // currently shown) local to this screen instance — navigating months only
  // ever re-renders the #assessor-cal-wrap contents below, not the whole
  // assessor screen, so it doesn't re-fetch results or collapse any expanded
  // session rows elsewhere on the page.
  const calViewDate = new Date();
  calViewDate.setDate(1);

  function refreshCalendar() {
    const wrap = document.getElementById("assessor-cal-wrap");
    if (!wrap) return;
    wrap.innerHTML = buildAssessorCalendarInnerHtml(calendarByDay, calViewDate.getFullYear(), calViewDate.getMonth());
    const prevBtn = document.getElementById("assessor-cal-prev");
    const nextBtn = document.getElementById("assessor-cal-next");
    if (prevBtn) {
    ...
    wrap.querySelectorAll("[data-cal-day]").forEach((cell) => {
      cell.addEventListener("click", () => showAssessorCalendarDayModal(cell.dataset.calDay, calendarByDay[cell.dataset.calDay]));
    });
  }
  refreshCalendar();
